...
const Products = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      try {
        ...
        // Client-side filtering for category if selected
        if (selectedCategory) {
          ...
          productsWithCategoryNames = productsWithCategoryNames.filter(product => {
            ...
            switch(selectedStockFilter) {
              ...
              case 'out_of_stock':
                return !inStock || stock === 0;
              case 'low_stock':
                return inStock && stock > 0 && stock <= 5;
              default:
                return true;
            }
          });
  ...
  const handleSectionToggle = async (productId, section) => {
    try {
      ...
      await productService.updateProductSections(productId, {
      ...
      setError(error.message || `Failed to toggle ${section}`);
    }
  };
