...
const HeroCarousel = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await heroCarouselService.getCarouselItems();
      setItems(response.data || []);
    } catch (error) {
      console.error("Failed to fetch carousel items", error);
      setError(error.message || "Failed to load carousel items. Please try again later.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this carousel item?")) {
      try {
        setLoading(true);
        setError(null);
        await heroCarouselService.deleteCarouselItem(id);
        await fetchItems();
      } catch (error) {
        console.error("Failed to delete carousel item", error);
        setError(error.message || "Failed to delete carousel item. Please try again later.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleToggleActive = async (id) => {
    try {
      setToggleLoading(id);
      setError(null);
      await heroCarouselService.toggleCarouselActive(id);
      await fetchItems();
    } catch (error) {
      console.error("Failed to toggle carousel item status", error);
      setError(error.message || "Failed to update carousel item status. Please try again later.");
    } finally {
  ...
  const handleMove = async (currentIndex, direction) => {
    ...
    try {
      setError(null);
      await heroCarouselService.updateCarouselOrder(newItems);
      await fetchItems();
    } catch (error) {
    ...
    <div className="p-6">
      ...
      {!loading && filteredItems.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No carousel items found</h3>
