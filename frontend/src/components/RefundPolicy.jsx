...
const RefundPolicy = () => {
  ...
  const fetchPolicy = async () => {
    try {
      setLoading(true);
      const response = await policyService.getPolicyByType('refund');
          ...
          <div className="bg-gray-900 rounded-xl p-6 md:p-8 border border-gray-800">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm">The refund policy has not been set up yet.</p>
              </div>
            )}
          </div>
        </div>
