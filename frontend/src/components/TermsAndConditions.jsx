...
const TermsAndConditions = () => {
  ...
  const fetchPolicy = async () => {
    try {
      setLoading(true);
      const response = await policyService.getPolicyByType('terms');
      ...
      <div className="w-full bg-black border-b border-gray-800 py-6 md:py-8">
        <div className="w-full px-4 md:px-6 lg:px-8">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-center">
            {policy?.heading || 'Terms & Conditions'}
          </h2>
          ...
          <div className="bg-gray-900 rounded-xl p-6 md:p-8 border border-gray-800">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm">The terms and conditions have not been set up yet.</p>
              </div>
            )}
          </div>
        </div>
